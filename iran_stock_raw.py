...
class IranStock:
    ...
    def _get_raw_data(self):
        ...
        self.raw_data = np.zeros((len(self._dates), len(self._instrument_ids)))
        for entry in entries:
            ...
            amount = entry[2]
            self.raw_data[time_frame_index, node_index] = amount

    def _fill_raw_data_empty_entries(self):
        for column_index in range(self.raw_data.shape[1]):
            previous_value = 0
            for row_index in range(self.raw_data.shape[0]):
                if not self.raw_data[row_index, column_index]:
                    self.raw_data[row_index, column_index] = previous_value
                else:
                    previous_value = self.raw_data[row_index, column_index]
    ...
    def _delete_static_columns(self):
        normalized_x = self._subtract(self.raw_data)
...
if __name__ == '__main__':
    iran_stock = IranStock()
    print(iran_stock.raw_data.shape)
    print(iran_stock.raw_data)
